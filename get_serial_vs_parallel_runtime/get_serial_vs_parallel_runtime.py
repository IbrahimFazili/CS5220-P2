...
# # Function to parse output files and extract execution times
# def parse_output_files(file_suffix):
#     results = []
#     for h in h_values:
#         output_file = f"./O(n)_timing_out/sph_h_{h}_{file_suffix}.out"
...
#                         num_particles = estimate_num_particles(h)
#                         results.append((h, num_particles, execution_time))
#                         print(f"{file_suffix} h = {h}, N = {num_particles}, time = {execution_time}s")
#         else:
#             print(f"Warning: {output_file} not found.")
#     return results

# # Parse both sets of files: parallel and serial
# parallel_results = parse_output_files("parallel")
# serial_results = parse_output_files("serial")

# # Convert results to numpy arrays for plotting
# h_vals_parallel, n_vals_parallel, times_parallel = zip(*parallel_results)
# h_vals_serial, n_vals_serial, times_serial = zip(*serial_results)

# n_vals_parallel = np.array(n_vals_parallel)
# times_parallel = np.array(times_parallel)

# n_vals_serial = np.array(n_vals_serial)
# times_serial = np.array(times_serial)

# # Plot the results on a log-log scale
# plt.figure()
# plt.loglog(n_vals_parallel, times_parallel, '-o', label='SPH Parallel Runtime')
# plt.loglog(n_vals_serial, times_serial, '-s', label='SPH Serial Runtime')
# plt.xlabel('Number of Particles (N)')
# plt.ylabel('Execution Time (seconds)')
# plt.title('SPH Runtime vs Number of Particles (Parallel vs Serial)')
# plt.grid(True)
# plt.legend()

# # Save the plot
# plt.savefig('sph_parallel_vs_serial_scaling_plot.png')
# plt.show()
