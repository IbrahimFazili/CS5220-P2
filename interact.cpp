...
void compute_density(sim_state_t* s, sim_param_t* params)
{
    ...
    #pragma omp parallel
    {
        // avoid thread conflicts with private vars
        float local_rho_i = 0.0f; // local contribution for particle i

        #pragma omp for schedule(dynamic,64) nowait
        for (int i = 0; i < n; ++i) {
            particle_t* pi = &p[i];
            local_rho_i = (315.0 / 64.0 / M_PI) * s->mass / h3; 
            // store positions in registers (for faster access)
            ...
            // use atomic to accumulate contributions (avoid performance bottlenecks from critical)
            #pragma omp atomic
            p[i].rho += local_rho_i; 
        }
    }
