...
mkdir -p $HOME/CS5220-P2/vtune_results_threading_cpu_{h_value}
vtune -collect threading -result-dir $HOME/CS5220-P2/vtune_results_threading_cpu_{h_value} -- srun ./sph.x
"""
...
def finalize_vtune_results(result_dir, h):
    ...
    report_file = f"/global/homes/a/acs378/CS5220-P2/poor_cpu_util_wait_vs_num_threads/summary_{h}_thread.csv"
...
for h in h_values:
    finalize_vtune_results(f"/global/homes/a/acs378/CS5220-P2/vtune_results_threading_cpu_{h}", h)
...
for filename in os.listdir('/global/homes/a/acs378/CS5220-P2/poor_cpu_util_wait_vs_num_threads'):
    if h_value_pattern.match(filename):
        ...
        file_path = os.path.join("/global/homes/a/acs378/CS5220-P2/poor_cpu_util_wait_vs_num_threads", filename)
...
plt.xlabel('CPU Cores/Threads (h_value)')
plt.ylabel('Wait Time with Poor CPU Utilization (s)')
plt.title('Wait Time with Poor CPU Utilization vs CPU Cores/Threads')
plt.grid(True)
plt.legend()
plt.savefig('poor_cpu_util_wait_vs_num_threads/graph.png')

# Directory containing the summary files
directory = '/global/homes/a/acs378/CS5220-P2/poor_cpu_util_wait_vs_num_threads'  # Replace with the actual path
...
plt.grid(True)
plt.legend()
plt.savefig('poor_cpu_util_wait_vs_num_threads/total_elapsed_time_v_num_threads.png')
...
plt.grid(True)
plt.legend()
plt.savefig('poor_cpu_util_wait_vs_num_threads/normalized_wait_time_v_num_threads.png')
...
if 'Serial Time' in data.columns:
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('poor_cpu_util_wait_vs_num_threads/serial_time_v_num_threads.png')